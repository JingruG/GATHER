...
def ForwardModelsVal(args, task_cfg, device, task_id, batch, model, task_losses):
    ...
    features, spatials, image_mask, question, target, input_mask, segment_ids, co_attention_mask, question_id, \
        wiki_tokens, concept_tokens, google_images, verif_labels, verif_inds, verif_concepts, target_scores, target_mask, close_mask, p_open_mask, open_mask = batch
    ...
    vil_pred, knowledge_pred, wiki_pred, concept_pred, image_pred, knowledge_mavex_pred, wiki_mavex_pred, concept_mavex_pred, \
        image_mavex_pred, knowledge_mavex_mask, wiki_mavex_mask, concept_mavex_mask, image_mavex_mask,  \
        target_features, trans_answers, target_txt, pred_nodes, fullgraph_anchors, anchors, \
        pred_features, pruned_pred_features, end_point_idxs, start_point_idxs, pred_prob, indices_topk, endnode_indices_topk, keep_mask = model(
            question,
            features,
            spatials,
            segment_ids,
            input_mask,
            # batch, 4(search words), 10 (sents) 18 (tokens)
    ...
    # print('answer',trans_answers)
    # sigmoid = torch.nn.Sigmoid()
    # num_answers = wiki_mavex_pred.size(1)
    # final_pred = torch.stack([vil_pred, knowledge_pred], 1).max(1)[
    #     0]  # batch, num_search, 5020
    # pred_prob = sigmoid(torch.gather(final_pred, 1, verif_inds.long()))

    # vil_pred_np = vil_pred.detach().cpu().numpy()
    # vil_pred_idx_np = torch.topk(vil_pred, 10)[1].detach().cpu().numpy()
    # target_idx_np = torch.topk(target, 5)[1].detach().cpu().numpy()
    # target_prob_np = torch.topk(target, 5)[0].detach().cpu().numpy()
    ...
    # print('question_id', question_id, 'question', question[0])
    # print('vil_pred', torch.topk(vil_pred[0],10), vil_pred.shape)
    # print('target', torch.topk(target[0],5))
    # print('wiki_pred', torch.topk(wiki_pred[0],10))
    # print('concept_pred', torch.topk(concept_pred[0],10))
    # print('image_pred', torch.topk(image_pred[0],10))
    # print('knowledge_pred', torch.topk(knowledge_pred[0],10))

    # vil_pred = vil_pred.detach().cpu()
    ...
    # if qv_node_scores is not None:
    #     qv_node_scores = qv_node_scores.detach().cpu()
    # batch_score_final_pred = compute_score_with_logits(
    #     final_pred, target).sum()
    ...
    # batch_score_mavex_k = compute_score_with_logits(knowledge_mavex_pred[:, np.arange(
    #     num_answers), np.arange(num_answers)] * pred_prob, verif_labels).sum()
    # batch_score_mavex_w = compute_score_with_logits(wiki_mavex_pred[:, np.arange(
    #     num_answers), np.arange(num_answers)] * pred_prob, verif_labels).sum()
    # batch_score_mavex_c = compute_score_with_logits(concept_mavex_pred[:, np.arange(
    #     num_answers), np.arange(num_answers)] * pred_prob, verif_labels).sum()
    # batch_score_mavex_i = compute_score_with_logits(image_mavex_pred[:, np.arange(
    #     num_answers), np.arange(num_answers)] * pred_prob, verif_labels).sum()

    # nli_score, _ = compute_entailment_loss(statement_txt, trans_answers, target_txt)
    # nli_score, _ = compute_entailment_loss(statement_txt, pred_nodes, target_txt)
    # batch_graph_acc_score = compute_score_with_pred_txt(pred_nodes, target_txt)
    # batch_graph_acc_score = compute_score_with_pred_ids(pred_concept_ids, verif_concepts)
    # For Anchor-Accuracy
    # target_scores = torch.stack([torch.tensor([target_scores[i][idx] for idx in idxs]) for i, idxs in enumerate(indices_topk)])

    # endnode_indices_sorted = torch.sort(pred_prob, descending=True, dim=1).indices
    # indices_topk, endnode_indices_topk = [], []
    # for i in range(batch_size):
    #     end_points = batch_end_point_idxs[i][endnode_indices_sorted[i]]
    ...
    # indices_topk = torch.stack(indices_topk)
    # pred_nodes_topk = [[[self.id2concept[node_ids[i]] for i in batch_sampled_paths[row][idx.item()] if i>=0] for idx in idxs] for row, idxs in enumerate(pred_indices_topk)] # node path
    # endnode_indices_topk = torch.stack(endnode_indices_topk)
    ...
    if keep_mask is not None:
        ...
        # loss_prune = triplet_loss(fullgraph_anchors, pred_features, target_scores, semi=False, woanchor=False)

        a = len((target_scores==1.).int().sum(1).nonzero())
        target_scores = target_scores[keep_mask==1.].reshape(keep_mask.shape[0], -1)

        # for i in range(target_scores.shape[0]):
        #     if sum(target_scores[i]) <= 0:
        #         target_scores[i][torch.randint(0,target_scores.shape[1], (1,))] = 0.0001
    if 'path' in args.ablation and 'path_pool' not in args.ablation:
        batch_path_acc_score_1, batch_path_acc_score_5, batch_path_acc_score_10, batch_path_acc_score_20, batch_close_acc_score_1, batch_partial_open_acc_score_1, batch_open_acc_score_1 = ComputeAccuracy10(None, target_scores, indices_topk, close_mask, p_open_mask, open_mask)
        batch_acc_score_1, batch_acc_score_10, batch_acc_score_20, batch_acc_score_50, batch_acc_score_100, batch_close_acc_score_1, batch_partial_open_acc_score_1, batch_open_acc_score_1 = ComputeAccuracy100(None, target_scores, endnode_indices_topk, close_mask, p_open_mask, open_mask)
    else:
        batch_acc_score_1, batch_acc_score_10, batch_acc_score_20, batch_acc_score_50, batch_acc_score_100, batch_close_acc_score_1, batch_partial_open_acc_score_1, batch_open_acc_score_1 = ComputeAccuracy100(
            None, target_scores, indices_topk,  close_mask, p_open_mask, open_mask)
    ...
    #     a = len((target_scores==1.).int().sum(1).nonzero())
    #     target_scores = target_scores[keep_mask==1.].reshape(keep_mask.shape[0], -1)
    #     # print('target_scores: %d in  %d / %d retrieved,  %d / %d has gt' % (len((target_scores==1.).int().sum(1).nonzero()), len(torch.unique(target_scores.nonzero(as_tuple=True)[0])), a,  a, target_scores.shape[0]))
        
    #     # for i in range(target_scores.shape[0]):
    #     #     if sum(target_scores[i]) <= 0:
    #     #         target_scores[i][torch.randint(0,target_scores.shape[1], (1,))] = 0.001

    # if 'path' in args.ablation:
    #     path_scores, resampled_path_scores, resampled_pred_prob, resampled_pred_features = [], [], [], []
    #     c = 0
    #     k = int(end_point_idxs.size(1) * 0.9)
    #     c_pos = 0
    #     for i in range(target_scores.shape[0]):
    #         t = target_scores[i,:]
    ...
    #         target_scores = torch.stack(resampled_path_scores)
    #         pred_prob = torch.stack(resampled_pred_prob)
    #         pruned_pred_features = torch.stack(resampled_pred_features)
    #     else:
    #         target_scores = torch.stack(path_scores)
    # if 'tri' in args.ablation:
    #     triplet_loss = TripletLoss(margin=0.2)
    #     loss_tri = triplet_loss(anchors, pruned_pred_features, target_scores, semi=False, woanchor=True)
    #     # triplet_loss = SupContrastiveLoss()
    #     # loss_tri = triplet_loss(pruned_pred_features, target_scores)
    #     loss_cls = loss_tri
    # if 'bce' in args.ablation:
    #     pred_prob = pred_prob / 0.1 
    #     # print('pred_prob', pred_prob)
    #     # loss_cls = LossMap['CrossEntropyLoss'](pred_prob, target_scores)
    #     wbce_loss = W_BCEWithLogitsLoss( 1 - 1 / target_scores.shape[1])
    #     loss_cls = wbce_loss(pred_prob, target_scores)
    #     if 'tri' not in args.ablation: 
    #         loss_tri = loss_cls
    # if keep_mask is None:
    #     loss_prune = loss_tri 
    # print('loss_prune', loss_prune, 'loss_tri', loss_tri, 'loss_cls', loss_cls )
    ...
    for i in range(batch_size):
        results.append({
                        ...
                        # 'wiki_pred': wiki_pred_np[i], 'knowledge_pred': knowledge_prediction_np[i],
    ...
    if 'path' in args.ablation:
        ...
        returned_variables = ['batch_size', 'results', 'batch_acc_score_1', 'batch_acc_score_10'
                            , 'batch_acc_score_20', 'batch_acc_score_50', 'batch_acc_score_100', 'batch_close_acc_score_1', 'batch_partial_open_acc_score_1', 'batch_open_acc_score_1']
    return_dict = {}
...
def ForwardModelsTrain(
    ...
    # subgraph = subgraph.cuda(device=device, non_blocking=True)
    ...
    features, spatials, image_mask, question, target, input_mask, segment_ids, co_attention_mask, question_id, \
        wiki_tokens, concept_tokens, google_images, verif_labels, verif_inds, verif_concepts, target_scores, target_mask, _, _, _ = batch
    ...
    # vil_pred_idx_np = torch.topk(vil_pred, 10)[1].detach().cpu().numpy()[0]
    # target_idx_np = torch.topk(target, 5)[1].detach().cpu().numpy()[0]
    # target_prob_np = torch.topk(target, 5)[0].detach().cpu().numpy()[0]
    ...
    # target_txt_encodings = model.small_bert_tokenizer(target_txt, padding=True, return_tensors='pt')
    # target_txt_embeds = model.small_bert_model(**target_txt_encodings)
    # loss_qa = loss_fn(trans_answers_embeds, target_txt_embeds).mean() * 100
    # loss_knowledge_pred = loss_qa

    # _, loss_nli = compute_entailment_loss(statement_txt, gt_answers, trans_answers)

    # Compute cross entropy between predicted graph logits and target bert embedding
    # loss_entity = LossMap['CrossEntropyLoss'](pred_features, target_features)*100
    ...
    if 'path' in args.ablation and 'path_pool' not in args.ablation:
        path_scores, resampled_path_scores, resampled_pred_prob, resampled_pred_features = [], [], [], []
        # qv node scores
        # qv_node_scores = (0.8+0.1*qv_node_scores).reshape(target_scores.shape[0], -1)
        c = 0
        ...
        if len(resampled_path_scores) == len(path_scores):
            ...
            target_scores = torch.stack(path_scores)
        print('no gt path: %d / %d, %d / %d positive' % (c, target_scores.shape[0], int(c_pos/target_scores.shape[0]), target_scores.shape[1]))
        # print('gt_paths: %.2f / %d' % (target_scores.sum(), target_scores.shape[0]*target_scores.shape[1]))
        # print('target_scores', target_scores.shape)

    if 'tri' in args.ablation:
        # triplet_loss = MultipleNegativesRankingLoss(scale=20.0)
        triplet_loss = TripletLoss(margin=0.5)
        loss_tri = triplet_loss(anchors, pruned_pred_features, target_scores, semi=True, woanchor=False, minneg=True)
        # triplet_loss = SupContrastiveLoss()
        # loss_tri = triplet_loss(pruned_pred_features, target_scores)
        loss_cls = loss_tri
    ...
    if 'bce' in args.ablation:
        # pred_prob = pred_prob / 0.1 
        # print('pred_prob', pred_prob)
        # loss_cls = LossMap['BCEWithLogitLoss'](pred_prob, target_scores).sum(1).mean()
        criterion_ce = nn.CrossEntropyLoss()
        loss_cls = criterion_ce(pred_prob, target_scores)
        # if MSELoss
        # criterion_mse = nn.MSELoss()
        # loss_prune = 0
        # for i in range(target_scores.shape[0]):
        #     for pos_emb in pruned_pred_features[i][target_scores[i] > 0]:
    ...
    if keep_mask is None and 'tri' in args.ablation:
        loss_prune = loss_tri 

    if 'classification' in args.ablation:
        target_scores = verif_inds
        ...
        #     for pos_emb in pruned_pred_features[i][target_scores[i] > 0]:
        #         loss_prune += criterion_mse(anchors[i], pos_emb)
        loss_cls = criterion_cls(pred_prob, target_scores)
        loss_tri = loss_cls
        # triplet_loss = TripletLoss(margin=0.1)
        # loss_tri = triplet_loss(anchors, pruned_pred_features, target_scores, semi=False, woanchor=False, minneg=True)        
        loss_prune = loss_tri 


    # loss_tri = loss_cls
    # triplet_loss = TripletLoss(margin=1.0)
    # loss_cls = triplet_loss(anchors, pruned_pred_features, target_scores, end_point_idxs)
    # loss_cls = pairwise_hinge(pred_prob, target_scores)
    # for p,t in zip (pred_prob, target_scores):
    #     loss_cls += ranknet_grad(p.unsqueeze(0),t.unsqueeze(0))
    # loss_cls = sum(loss_cls)
    # one = torch.ones_like(target_scores)
    # target_scores = torch.where(target_scores > 0, one, target_scores).type(torch.LongTensor).cuda()
    # loss_cls = LossMap['CrossEntropyLoss'](pred_features.permute(0,2,1), target_scores)

    # print('indices_topk', indices_topk[0],pred_nodes[0][0], pred_prob[0][:10])
    # loss_entity = pr_loss(pred_features, target_scores) * 100
    # triplet_loss = nn.TripletMarginLoss(margin=1.0, p=2)

    # Compute Entity loss
    # loss_concept_diag = mis_match_loss_max(pred_nodes, verif_concepts)

    # loss_knowledge_pred = loss_concept_pred + loss_wiki_pred + loss_knowledge_pred + loss_image_pred
    # loss_prune = loss_tri
    loss_vqa = loss_prune + loss_tri + loss_cls #+ loss_tri_qa
    ...
    loss = loss_vqa  # + loss_knowledge_pred + loss_mavex  #+ loss_nli*1000
    return_dict = {}
    returned_variables = ['loss', 'loss_vqa', 'loss_tri', 'loss_prune', 'loss_cls']
    for name in returned_variables:
...
def ComputeAccuracy100(logits, y, indices_topk, close_mask, p_open_mask, open_mask):
    # logits = logits.detach()
    y = torch.stack([torch.tensor([y[i][idx] for idx in idxs])
...
def ComputeAccuracy10(logits, y, indices_topk, close_mask, p_open_mask, open_mask):
    y = torch.stack([torch.tensor([y[i][idx] for idx in idxs])
                    for i, idxs in enumerate(indices_topk)]).cuda()
