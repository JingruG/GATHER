from vilbert.modeling.modeling_encoder import TextEncoder, MODEL_NAME_TO_CLASS
from vilbert.gnnutils.data_utils import *
from vilbert.gnnutils.layers import *
...
class QAGNN_Message_Passing(nn.Module):
    ...
    def mp_helper(self, _X, Y1, Y2, edge_index, edge_type, _node_type, _node_feature_extra):
        for _ in range(self.k):
            _X = self.gnn_layers[_](_X, Y1, Y2, edge_index, edge_type, _node_type, _node_feature_extra)
            _X = self.activation(_X)
            _X = F.dropout(_X, self.dropout_rate, training = self.training)
        return _X


    def forward(self, H, Q1, Q2, A, node_type, node_score, cache_output=False):
        """
        ...
        Y1 = Q1.reshape(-1, Q1.size(2)) 
        Y2 = Q2.reshape(-1, Q2.size(2)) 
        ...
        _X = self.mp_helper(_X, Y1, Y2, edge_index, edge_type, _node_type, _node_feature_extra)
...
class GATConvE(MessagePassing):
    ...
    def __init__(self, args, emb_dim, n_ntype, n_etype, edge_encoder, head_count=4, aggr="add"):
        ...
        #For final MLP
        self.mlp = torch.nn.Sequential(torch.nn.Linear(emb_dim*2, emb_dim), torch.nn.BatchNorm1d(emb_dim), torch.nn.ReLU(), torch.nn.Linear(emb_dim, emb_dim))


    def forward(self, x, y1, y2, edge_index, edge_type, node_type, node_feature_extra, return_attention_weights=False):
        # x: [N, emb_dim]
        # edge_index: [2, E]
        ...
        y1 = torch.cat([y1, node_feature_extra], dim=1)
        y2 = torch.cat([y2, node_feature_extra], dim=1)
        x = (x, x)
        y1 = (y1, y1)
        y2 = (y2, y2)
        aggr_out1 = self.propagate(edge_index, x=x, y=y1, edge_attr=edge_embeddings) #[N, emb_dim]
        aggr_out2 = self.propagate(edge_index, x=x, y=y2, edge_attr=edge_embeddings) #[N, emb_dim]
        # print('aggr_out2', aggr_out2.shape)
        aggr_out = torch.cat((aggr_out1, aggr_out2), 1)
    ...
    def message(self, edge_index, x_i, x_j, y_i, y_j, edge_attr): #i: tgt, j:src
        ...
        query = self.linear_query(y_j).view(-1, self.head_count, self.dim_per_head) #[E, heads, _dim]
